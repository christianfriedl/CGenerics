...
\
void ArrayOf##TYPENAME##_insertValueAtAfterEnd_(AppState* appState, ArrayOf##TYPENAME* this, const TYPENAME value, const unsigned int at) { \
...
\
void ArrayOf##TYPENAME##_insertValueAt(AppState* appState, ArrayOf##TYPENAME* this, const TYPENAME value, const unsigned int at) { \
    if (at >= this->usedElements) { \
...
\
unsigned int ArrayOf##TYPENAME##_searchIndex(AppState *appState, ArrayOf##TYPENAME* this, TYPENAME* elementPointer, int (comparingFunction)(const TYPENAME *, const TYPENAME *)) { \
    unsigned int i; \
    for (i = 0; i < this->usedElements; ++i) \
        if ((comparingFunction)((this->vector + i), elementPointer) == 0) \
            return i; \
