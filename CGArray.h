...
CGArrayOf##TYPENAME* CGArrayOf##TYPENAME##__new(CGAppState* appState, const unsigned int initialCapacity) { \
    ...
    if (this != NULL) { \
        ...
        if (this->vector == NULL) \
            CGAppState_throwException(appState, Exception__new(Severity_error, ExceptionID_CannotAllocate, "cannot allocate CGArray")); \
    } else \
        CGAppState_throwException(appState, Exception__new(Severity_error, ExceptionID_CannotAllocate, "cannot allocate CGArray")); \
    return this; \
} \
...
TYPENAME* CGArrayOf##TYPENAME##_getValueAt(CGAppState* appState, CGArrayOf##TYPENAME* this, const unsigned int at) { \
    if (at >= this->usedElements) { \
        CGAppState_throwException(appState, Exception__new(Severity_error, ExceptionID_CGArrayIndexOutOfBounds, "cannot get value beyond array end (requested position: %u)", at)); \
        return *(this->vector); \
    } \
...
TYPENAME* CGArrayOf##TYPENAME##_pop(CGAppState* appState, CGArrayOf##TYPENAME* this) { \
    if (this->usedElements == 0) { \
        CGAppState_throwException(appState, Exception__new(Severity_error, ExceptionID_CGArrayIndexOutOfBounds, "cannot pop from empty array")); \
        return *(this->vector); \
    } \
    --this->usedElements; \
...
TYPENAME* CGArrayOf##TYPENAME##_shift(CGAppState* appState, CGArrayOf##TYPENAME* this) { \
    if (this->usedElements == 0) {\
        CGAppState_throwException(appState, Exception__new(Severity_error, ExceptionID_CGArrayIndexOutOfBounds, "cannot shift from empty array")); \
        return *(this->vector); \
    } \
...
TYPENAME* CGArrayOf##TYPENAME##_removeValueAt(CGAppState* appState, CGArrayOf##TYPENAME* this, unsigned int at) { \
    if (at >= this->usedElements) { \
        CGAppState_throwException(appState, Exception__new(Severity_error, ExceptionID_CGArrayIndexOutOfBounds, "cannot remove value beyond array end")); \
        return *(this->vector); \
    } \
